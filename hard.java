...
class MemoryGameHard extends JPanel {

    static String files[] = {"plant.png", "starfish.png", "hat.png","kaprun.png","coconut.png","crab.png","fish.png","bird.png","chell.png"};

    ImageIcon gameover = new ImageIcon(this.getClass().getResource("gameover1.gif"));
    ImageIcon Home_Screen = new ImageIcon(this.getClass().getResource("hard_star.gif"));
    ImageIcon Hard = new ImageIcon(this.getClass().getResource("Hard.png"));
    ImageIcon catmagic = new ImageIcon(this.getClass().getResource("catmagic.gif"));
    ...
    int oddClickIndex = 0;
    int currentIndex = 0;
    boolean aa[] = {false,false,false,false,false,false,false,false,false};
    int x = 0;
...
// Create a BorderLayout manager.
        ...
        closedIcon = new ImageIcon(this.getClass().getResource("mirror.png"));
...
// randomize icons array
        ...
        for (int i = 0; i < numButtons; i++) {
            int rand = gen.nextInt(numButtons);//randome
...
// show images for 1 sec, before flipping back
                    ...
                    aa[x] = true; //array boolean
                    x++;//next array or score
                }
            } else {
// we just record index for odd clicks
    ...
    protected void paintComponent(Graphics g) {
        ...
        g.drawImage(catmagic.getImage(), 140, 500, 210, 210, this);
        g.drawImage(Hard.getImage(), 0, -34, 990, 790, this);
        g.setFont(new Font("Monospaced", Font.PLAIN, 64));
        g.setColor(Color.WHITE);
        g.drawString(String.valueOf(x) + " SCORE ", 500, 600);//point score
        if (Second < 10) {
        ...
        if (Second == 0) {
            ...
            Draw.stop();//stop paint 
            myTimer.stop();
            start.setFocusable(true);
        }
        if (aa[0]==true&&aa[1]==true&&aa[2]==true&&aa[3]==true&&aa[4]==true&&aa[5]==true&&aa[6]==true&&aa[7]==true&&aa[8]==true) {
            for (int i = 0; i < files.length*2; i++) {
                remove(buttons[i]);
            }
    ...
    class gamewin extends JPanel {
        ...
        protected void paintComponent(Graphics g) {
            ...
            g.setColor(Color.white);
            g.drawString(String.valueOf(90- Second) + " SECOUND", 100, 700);
            g.setColor(Color.white);
